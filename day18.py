inputs = []
inputs.append("""[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]
[[[[5,4],[7,7]],8],[[8,3],8]]
[[9,3],[[9,9],[6,[4,9]]]]
[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]""")

inputs.append("""[[7,[8,[3,5]]],[[[3,6],9],1]]
[[[[1,7],8],[0,4]],[[[0,9],2],[2,[5,6]]]]
[[[4,[4,4]],[8,[4,4]]],[[5,4],6]]
[[[1,[1,3]],[[9,6],1]],[[4,[5,4]],[[4,4],[0,8]]]]
[[[[6,2],[2,5]],[2,1]],[[1,5],7]]
[[[[5,0],[2,7]],[[2,5],2]],[2,[3,2]]]
[[[6,[6,6]],[0,[2,8]]],[[8,[5,6]],[4,5]]]
[[[6,[3,5]],8],[[2,[7,0]],5]]
[[[[7,8],[3,6]],[1,6]],[[[4,2],1],[[0,7],[5,6]]]]
[8,2]
[[5,5],[[2,[9,1]],8]]
[[[4,8],[[1,8],2]],[9,2]]
[2,[[8,[8,3]],[0,6]]]
[[[6,6],[[6,0],6]],[0,[[3,4],3]]]
[[[[2,9],[5,9]],[2,[4,3]]],[6,0]]
[[[6,2],0],[7,7]]
[[[9,6],5],[2,[[0,1],[5,5]]]]
[[6,[[0,1],[5,1]]],5]
[4,[[[4,2],3],[2,[5,0]]]]
[[[7,9],2],2]
[[[5,[2,1]],1],[[1,1],[8,5]]]
[[[[5,9],0],[[1,9],0]],4]
[[7,[0,5]],[[0,3],[8,2]]]
[[6,[9,[7,7]]],6]
[2,[[1,[1,0]],[4,[6,1]]]]
[[0,6],[[[5,1],6],[[4,7],[8,0]]]]
[[[1,[4,7]],[0,[1,2]]],[[1,1],[[1,2],[1,3]]]]
[[8,[3,0]],[3,[1,[8,1]]]]
[[[7,[4,0]],[[8,7],2]],[[7,[7,3]],7]]
[3,[[1,7],2]]
[8,[[[1,5],0],1]]
[[6,[1,4]],7]
[[[[2,6],2],8],1]
[9,7]
[9,[[[1,1],1],[[3,0],[7,3]]]]
[[[[8,5],7],[[5,1],[6,4]]],[4,[[7,6],[2,7]]]]
[[[[8,7],1],0],[[9,9],[[1,0],8]]]
[[9,[[1,1],7]],[[3,0],4]]
[[[[8,2],4],[9,[7,9]]],[[0,2],[[3,0],5]]]
[[[[3,6],3],[[9,7],[0,6]]],[[[4,9],[1,3]],[2,[7,3]]]]
[[[[3,8],0],[[3,6],5]],[[3,[4,2]],[[6,1],[8,5]]]]
[[2,7],[[0,0],8]]
[[[[0,3],7],[2,0]],3]
[[0,2],[[[3,1],0],[0,0]]]
[[[[6,1],7],[[8,4],[2,4]]],[[1,6],[2,3]]]
[[2,[2,[9,1]]],[4,[[0,4],9]]]
[[[3,[5,6]],7],1]
[[[3,0],[8,[9,3]]],[[[1,1],1],[6,7]]]
[[[6,[4,4]],[[1,9],1]],[[[8,1],[9,8]],[[6,3],1]]]
[[[5,8],[[0,3],[1,7]]],[[[3,2],[4,7]],1]]
[[5,5],[[[8,3],[6,6]],2]]
[[[[1,9],[8,5]],[[7,7],8]],[0,[8,[7,4]]]]
[[6,[4,[4,3]]],[5,[6,[7,2]]]]
[[0,[[9,0],0]],5]
[[[[5,6],[1,3]],[[0,5],[7,5]]],[[[0,4],[3,6]],[8,[3,6]]]]
[[3,[[4,7],[7,0]]],[[[4,1],5],[[6,6],[7,4]]]]
[[[[4,3],[0,1]],[[7,3],[2,3]]],[[[3,7],[2,2]],[6,5]]]
[[1,1],[[[1,4],6],[6,[3,9]]]]
[[[[0,8],[2,0]],5],[4,[[6,1],[2,1]]]]
[[7,[3,[7,2]]],[[7,9],8]]
[[[4,[9,8]],[8,[3,2]]],[7,9]]
[[[4,[4,2]],[5,[0,3]]],[[[4,9],[2,9]],[[1,5],[0,8]]]]
[[1,[[9,8],0]],[5,[[4,3],5]]]
[[[[5,1],3],[[2,9],[9,0]]],[1,[6,3]]]
[[[6,4],[6,1]],7]
[[4,8],[[7,2],6]]
[[[5,[4,8]],[[1,7],[2,8]]],[6,[[8,4],[3,5]]]]
[[5,8],[[[4,0],[6,0]],[5,[6,0]]]]
[[3,[[5,3],8]],[8,5]]
[[[2,6],[1,[2,3]]],[[[1,7],[5,7]],[[0,0],[0,5]]]]
[[0,[[4,3],[3,6]]],[[2,[6,6]],[0,[2,9]]]]
[[[5,9],[6,2]],[[7,6],8]]
[[9,2],[1,[[0,5],[5,0]]]]
[[[3,1],[9,3]],3]
[[[[2,0],[4,2]],6],[[[5,2],[7,8]],[[0,7],3]]]
[[7,[[3,9],[6,3]]],[2,[[6,4],3]]]
[[5,[3,[7,4]]],[[2,5],[0,9]]]
[3,7]
[[3,9],[[[4,4],9],[[3,1],7]]]
[[[[4,0],1],[8,[3,6]]],[[9,[4,4]],[[9,9],9]]]
[[8,[[8,1],5]],[[[9,1],4],[[8,5],3]]]
[[6,[[6,3],[3,7]]],4]
[[[1,[0,8]],9],[[8,5],[3,[0,5]]]]
[[[[3,1],0],[[8,5],[1,0]]],[0,2]]
[[2,[4,7]],2]
[[[2,0],[2,2]],4]
[4,[[5,8],5]]
[[[2,[0,5]],[[3,3],[6,6]]],1]
[[[2,[2,4]],[5,[7,1]]],[3,5]]
[[2,[9,[3,9]]],9]
[[[7,[7,1]],[[5,2],1]],[[2,1],[9,[7,3]]]]
[[4,[4,6]],4]
[[[4,2],[9,[3,8]]],[[2,4],0]]
[[[7,[0,3]],4],[[[1,8],4],[[5,1],1]]]
[[[1,3],3],[[4,9],[[0,0],6]]]
[[[4,1],0],[[[5,6],[0,8]],[[2,1],1]]]
[[3,[3,[7,9]]],[[[6,8],8],[[7,9],3]]]
[[4,[[1,6],[4,6]]],[[1,8],[3,8]]]
[[[[5,9],2],[[6,7],4]],3]
[[[[2,1],[1,9]],7],[[[0,9],[0,5]],[[2,5],[5,0]]]]""")

lines = inputs[1].split()

print lines
class Node:
    value = None
    right = None
    left = None

    def __init__(self, val=None):
        self.value = val

    def __str__(self):
        if self.value is not None:
            return str(self.value)
        else:
            return '[' + str(self.left) + ','+ str(self.right) + ']'

def parse(snailfish_string):
    try:
        return Node(val=int(snailfish_string))
    except:
        pass

    idx = 0
    balance = 0
    left = None
    n = Node()
    for c in snailfish_string[1:]:
        if c == '[':
            balance += 1
        elif c == ']':
            balance -= 1
        elif balance == 0 and c == ',':
            left = parse(snailfish_string[1:idx+1])
            idx += 1
            break
        idx += 1
    n.left = left
    n.right = parse(snailfish_string[idx+1:-1])
    return n 

snailnumbers = []
for line in lines:
    snailnumbers.append(parse(line))

def explode(node):

    def add_dir(direction, node, value):
        if node.value is not None:
            node.value += value
            return
        return add_dir(direction, node.left if direction == 'left' else \
                node.right, value)

    def helper(node, depth):
        if node.value is not None:
            return False, 0, 0

        if depth >= 4 and node.left.value is not None \
                and node.right.value is not None:
            left_value = node.left.value
            right_value = node.right.value
            node.left = None
            node.right = None
            node.value = 0
            return True, left_value, right_value 

        exploded, left_val, right_val = helper(node.left, depth + 1)
        if exploded:
            add_dir('left', node.right, right_val)
            return exploded, left_val, 0

        exploded, left_val, right_val = helper(node.right, depth + 1)
        if exploded:
            add_dir('right', node.left, left_val)
            return exploded, 0, right_val

        return False, 0, 0

    exploded, _, _ = helper(node, 0)
    return exploded

def split(node):

    if node.value is not None and node.value > 9:
        val = node.value
        node.value = None
        node.left = Node(val=val/2) 
        node.right = Node(val=val/2 if val % 2 == 0 else (val / 2) + 1) 
        return True
    elif node.value is not None:
        return False
    if split(node.left):
        return True
    if split(node.right):
        return True
    return False

def reduce(node):
    exploded = True
    splited = True
    while exploded or splited:
        exploded = explode(node)
        if exploded:
            continue
        splited = split(node)

def magnitude(node):
    if node.value is not None:
        return node.value
    return 3 * magnitude(node.left) + 2 * magnitude(node.right)


largest = 0
for i in range(len(snailnumbers)):
    for j in range(len(snailnumbers)):
        if i == j:
            continue
        a = parse(str(snailnumbers[i]))
        b = parse(str(snailnumbers[j]))

        p = Node()
        p.left = a 
        p.right = b 
        reduce(p)
        largest = max(largest, magnitude(p))
print largest



