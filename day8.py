inputs = []
inputs.append("""be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
inputs.append("""beacf afbd bcead cgefa ecdbga efb gbfdeac ecgfbd acbdfe fb | bf efb bgecdfa egcfa
...
for line in lines:
    ...
    inputs.sort(key=lambda elem: len(elem))
    inputs = map(lambda x: "".join(sorted(x)), inputs)
    real = {}
    real[inputs[0]] = 1
    real[inputs[1]] = 7 
    real[inputs[2]] = 4
    ...
    for five in inputs[3:6]:
        for l in five:
            if seen[l] == 1 and l not in bd:
                real[five] = 2
                e.add(l)

    for five in inputs[3:6]:
        if "".join(sorted(five)) not in real:
            real[five] = 3


    for six in inputs[6:9]:
        ding = 0
        bdcounter = 0
        for l in six:
            if l in bd:
                bdcounter += 1
            if l in bd or l in e:
                ding += 1
        if ding == 3:
            real[six] = 6
        elif bdcounter == 2:
    ...
    for i in range(len(digits)):
        out += mult * real["".join(sorted(digits[i]))]
        mult = mult / 10
    c += out
print c
